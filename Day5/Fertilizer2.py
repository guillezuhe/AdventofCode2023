...
for i in range(0, len(seeds_builder), 2):
    print('Original seed', seeds_builder[i])
    for j in range(0, seeds_builder[i+1]+1):
        mapped = seeds_builder[i] + j
        for map in maps:
            for range_ in map:
                ...
                if mapped >= source_start and mapped < (source_start + range_len):
                    mapped = destination_start + (mapped - source_start)
                    break
